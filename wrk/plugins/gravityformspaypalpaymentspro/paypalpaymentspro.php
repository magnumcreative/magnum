...
class GFPayPalPaymentsPro {
    ...
    private static function post_to_payflow($nvp = array(), $local_api_settings = array(), $form_id = null) {
        ...
        if(!empty($local_api_settings))
        {
            ...
            $Vendor = urlencode($local_api_settings["vendor"]);
            $Vendor = empty($Vendor) ? $API_UserName : $Vendor;
            $Partner = urlencode($local_api_settings["partner"]);
        ...
        else
        {
            ...
            $Vendor = urlencode(self::get_vendor());
            $Vendor = empty($Vendor) ? $API_UserName : $Vendor;
            $Partner = urlencode(self::get_partner());
            $Partner = empty($Partner) ? "PayPal" : $Partner;
            $mode = self::get_mode();
        }
    ...
    public static function paypalpaymentspro_commit_transaction($entry,$form){
        ...
        if($transaction_type == "2")
        {
            ...
            self::capture_subscription_payment($entry,self::$transaction_response["transaction_id"],$fee_amount,$config);

        }
        else
        {
            if(self::capture_product_payment($entry,$config))
                GFPayPalPaymentsProData::insert_transaction($entry["id"],$config["id"], "payment", $subscriber_id, self::$transaction_response["transaction_id"], $amount);
        }
    ...
    private static function capture_product_payment($entry,$config){
        ...
        {
            //Voiding original Authorization transaction
            $args = array("TRXTYPE" => "V", "TENDER" => "C", "ORIGID" => $origid);
            $void_response = self::post_to_payflow($args,$local_api_settings);
            return false;
        }
    }
    ...
    private static function capture_subscription_payment($entry,$transaction_id,$fee_amount,$config){
        ...
        //Voiding original Authorization transaction
        $args = array("TRXTYPE" => "V", "TENDER" => "C", "ORIGID" => $transaction_id);
        $void_response = self::post_to_payflow($args,$local_api_settings);
        ...
        if($response["RESULT"] == 0)
        {
            ...
            GFPayPalPaymentsProData::insert_transaction($entry["id"], $config["id"], "payment", $response["PROFILEID"], $response["PROFILEID"], $subscription["AMT"]);
            ...
            if(!empty($fee_amount) && $fee_amount > 0){
                ...
                GFPayPalPaymentsProData::insert_transaction($entry["id"], $config["id"], "payment", $response["PROFILEID"], $response["PROFILEID"], $fee_amount);
            }
